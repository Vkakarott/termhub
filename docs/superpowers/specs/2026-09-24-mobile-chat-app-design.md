...
Revoking on the web (or by the brute-force rule, or by an admin) sets `status = revoked`, deletes `device_tokens`, clears the push token and closes that device's `/ws/m/chat` sockets with code `4401`. The next call answers `DEVICE_REVOKED`; the app wipes key, secrets and PIN and returns to the first screen. "Sair e remover este aparelho" in the app is `POST devices/self/revoke` plus the same wipe. There is no "log out but keep the device".
...
| `POST chat/actions/:id/decision` | token (+ pin proof for approve) | `chat:create` | same `decide`, same `decision` event to every client |
...
| Account enumeration | Identical response, timing and polling: every request is a row, a decoy when the account does not exist or cannot enrol. |

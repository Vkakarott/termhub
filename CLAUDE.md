...
- Data is scoped by owner (`apps/server/src/auth/scope.ts`): machines and integrations have `owner_id`, everything under a machine inherits. In routes, load machines/projects/tabs/tasks/integrations/AI accounts through `scoped(repos, request).<kind>(id)` (404 outside the scope), pass `request.scope.ownerId` to list methods and `request.scope.createAs` as the owner of new rows. Never use `repos.*.findById` directly in a handler for these kinds.

...
const pt = {
  ...
  compare: {
    ...
    lead: 'Onde ele se encaixa entre um IDE de agentes no desktop, o SSH + tmux feito à mão e um IDE com IA.',
    columns: [
      { name: 'termhub', hint: '' },
    ...
    rows: [
      { label: 'Roda no navegador; nada a instalar no aparelho de acesso', cells: ['yes', 'no', 'no', 'no'] },
      { label: 'Várias máquinas em um só painel', cells: ['yes', 'partial', 'partial', 'partial'] },
      { label: 'Sessões sobrevivem ao navegador e à troca de computador (tmux)', cells: ['yes', 'partial', 'yes', 'no'] },
      { label: 'Acesso remoto por padrão, celular incluso', cells: ['yes', 'partial', 'partial', 'no'] },
      { label: 'Times, roles e permissões por recurso', cells: ['yes', 'no', 'no', 'partial'] },
      { label: 'Self-hosted e open source (MIT)', cells: ['yes', 'yes', 'yes', 'no'] },
      { label: 'Limites das contas de IA (Claude, Codex, Gemini)', cells: ['yes', 'yes', 'no', 'no'] },
    ],
  ...
  faq: {
    ...
    items: [
      { q: 'O que é o termhub?', a: 'Um painel no navegador para os terminais das suas máquinas. Você cadastra máquinas (local ou SSH), organiza por projeto e abre tabs; cada tab é uma sessão tmux na máquina de destino, que continua rodando quando você fecha o navegador.' },
      { q: 'Preciso instalar algo no computador que uso para acessar?', a: 'Não. Basta um navegador: funciona no notebook, no tablet e no celular. O que precisa existir é o servidor do termhub (um docker compose) e o tmux nas máquinas de destino.' },
      { q: 'É seguro? Onde ficam as credenciais?', a: 'O servidor usa uma chave SSH própria, que você autoriza em cada máquina. Para mostrar os limites das contas de IA, o login do CLI (Claude, Codex, Gemini, Antigravity) é lido na máquina na hora da consulta; nenhum token é armazenado e o conteúdo dos terminais nunca é logado.' },
      { q: 'Funciona com o Claude Code?', a: 'Sim, e foi feito pensando nele: colar imagens e arrastar arquivos direto no terminal, seleção e cópia mesmo com o app usando o mouse e locale UTF-8 garantido. Codex, Gemini CLI, Antigravity ou qualquer outra CLI rodam do mesmo jeito, porque é um terminal de verdade.' },
      { q: 'Funciona no Windows?', a: 'Para acessar, sim: é só abrir o navegador. Como máquina de destino, o termhub precisa de tmux, então use WSL ou uma máquina Linux/macOS.' },
      ...
      { q: 'O que é o termhub Cloud?', a: 'A mesma experiência sem servidor para manter: suas máquinas conectam ao Cloud e você acessa de qualquer lugar, com login, times e permissões prontos. Está em lista de espera.' },
...
const en: typeof pt = {
  ...
  compare: {
    ...
    lead: 'Where it fits next to a desktop agent IDE, hand-rolled SSH + tmux and an AI IDE.',
    columns: [
      { name: 'termhub', hint: '' },
    ...
    rows: [
      { label: 'Runs in the browser; nothing to install on the device you use', cells: ['yes', 'no', 'no', 'no'] },
      { label: 'Several machines in one dashboard', cells: ['yes', 'partial', 'partial', 'partial'] },
      { label: 'Sessions survive the browser and switching computers (tmux)', cells: ['yes', 'partial', 'yes', 'no'] },
      { label: 'Remote access by default, phone included', cells: ['yes', 'partial', 'partial', 'no'] },
      { label: 'Teams, roles and per-resource permissions', cells: ['yes', 'no', 'no', 'partial'] },
      { label: 'Self-hosted and open source (MIT)', cells: ['yes', 'yes', 'yes', 'no'] },
      { label: 'AI account limits (Claude, Codex, Gemini)', cells: ['yes', 'yes', 'no', 'no'] },
    ],
  ...
  faq: {
    ...
    items: [
      { q: 'What is termhub?', a: 'A browser dashboard for your machines’ terminals. You register machines (local or SSH), organize them by project and open tabs; each tab is a tmux session on the target machine that keeps running when you close the browser.' },
      { q: 'Do I need to install anything on the device I use to access it?', a: 'No. A browser is enough: it works on the laptop, the tablet and the phone. What must exist is the termhub server (one docker compose) and tmux on the target machines.' },
      { q: 'Is it secure? Where do the credentials live?', a: 'The server uses its own SSH key, which you authorize on each machine. To show AI account limits, the CLI login (Claude, Codex, Gemini, Antigravity) is read on the machine at query time; no token is stored and terminal content is never logged.' },
      { q: 'Does it work with Claude Code?', a: 'Yes, and it was built with it in mind: paste images and drop files straight into the terminal, select and copy even while the app owns the mouse, UTF-8 locale guaranteed. Codex, Gemini CLI, Antigravity or any other CLI run the same way, because it is a real terminal.' },
      { q: 'Does it work on Windows?', a: 'To access it, yes: just open the browser. As a target machine termhub needs tmux, so use WSL or a Linux/macOS machine.' },
      ...
      { q: 'What is termhub Cloud?', a: 'The same experience with no server to maintain: your machines connect to the Cloud and you reach them from anywhere, with sign-in, teams and permissions built in. It is waitlist-only for now.' },

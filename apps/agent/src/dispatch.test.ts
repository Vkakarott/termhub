...
import { createDispatcher, type PtyManager } from './dispatch.js';
...
describe('createDispatcher', () => {
  it('replies rpc_result ok:false code:invalid on bad params, echoing the id', async () => {
    ...
    const dispatch = createDispatcher({ handlers: makeHandlers(), pty: makePty(), log: vi.fn() });
  ...
  it('maps a handler throw to a generic internal error and logs the method (never params)', async () => {
    ...
    const dispatch = createDispatcher({ handlers, pty: makePty(), log });
  ...
  it('maps an RpcFailure to its own code/message/path', async () => {
    ...
    const dispatch = createDispatcher({ handlers, pty: makePty(), log: vi.fn() });
  ...
  it('routes open/resize/close to the pty manager with the same channel', async () => {
    ...
    const pty = makePty();
    const dispatch = createDispatcher({ handlers: makeHandlers(), pty, log: vi.fn() });
  ...
  it('logs (but does not throw or reply) when pty.open unexpectedly rejects', async () => {
    ...
    const dispatch = createDispatcher({ handlers: makeHandlers(), pty, log });

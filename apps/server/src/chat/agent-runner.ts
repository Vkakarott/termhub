...
export function agentRunner(machineId: string, opts: { host?: ClaudeChannelHost; deadlineMs?: number } = {}): RunnerClient {
  ...
  return {
    run: (input: RunnerInput) => {
      ...
      return runOnAgent(machineId, host, input, mcpUrl, opts.deadlineMs ?? RUN_DEADLINE_MS);
    },
  };
}
...
async function* runOnAgent(
  ...
  /** Lines the agent has sent and the caller has not read yet, and how the channel ended. Held on an
   *  object because both are written from the channel's callbacks, not from this generator's body. */
  const stream = {
    lines: [] as string[],
    /** Bytes after the last newline: the agent frames a line at a time, but a reader that assumed so
     *  would be one framing change away from gluing two lines together. */
    tail: '',
  ...
  // What `httpRunner` expresses with AbortSignal.timeout: here it ends the iteration and the `finally`
  // closes the channel, which kills the CLI on the machine too. A plain failed run — there is no
  // frame to read a reason from when the run simply never finished.
  ...
  try {
    // Channel data, never an open parameter: `claudeOpenParams` has no prompt field and its safeParse
    // strips unknown keys, so a prompt put in the open frame is dropped in silence and the run hangs
    // until a deadline. It is also the whole of the CLI's input — the agent closes stdin with it.
    ...
    // Every way out passes here — the deadline, an exception, a write that failed, and above all the
    // caller walking away (a `break` or a `return` inside its `for await` calls this generator's
    // `return`, which runs this block). Without it a CLI would keep running on someone's laptop with
    // nobody listening.
    clearTimeout(deadline);
    // Only a channel that is still open: a channel that reported its own end is gone from the
    // connection, and its number is free for the next terminal on that machine to be given — closing
    // it by number now could close that terminal instead.

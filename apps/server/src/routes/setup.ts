...
export async function setupRoutes(app: FastifyInstance, repos: Repositories) {
  ...
  app.put('/:id/setup', async (request) => {
    ...
    if (data.tickets && !(await repos.integrations.findById(data.tickets.integration_id))) throw badRequest('Integração de tickets inexistente');
    if (data.repo?.integration_id && !(await repos.integrations.findById(data.repo.integration_id))) throw badRequest('Integração do repositório inexistente');
    if (data.runner.machine_id && !(await repos.machines.findById(data.runner.machine_id))) throw badRequest('Máquina do runner inexistente');

/**
 * Cookie consent for the app's Google Analytics. Same contract and storage key as
 * the landing's `consent.ts`; the two are separate origins, so the choice is asked
 * once per host.
 *
 * Nothing from Google is loaded before the user accepts, so there is no Consent
 * Mode signalling to do: the choice simply gates `initAnalytics`.
 */
...
/** The stored choice, or null when the user has not answered (or storage is unavailable). */

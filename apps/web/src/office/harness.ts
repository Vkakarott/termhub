...
const rooms: OfficeRoom[] = Array.from({ length: roomCount }, (_, r) => {
  ...
  const tabs = Array.from({ length: n }, (_, i): OfficeTab => {
    ...
    return { id: `t${r}-${i}`, project_id: `p${r}`, name: i === 0 ? 'um nome de aba bem comprido mesmo 🚀' : `aba ${i + 1}`, kind: i % 8 === 7 ? 'simulator' : 'terminal', tmux_session: null, simulator_udid: null, position: i, state, state_text: null, state_tool: null, state_at: state ? at : null, state_seen_at: null, created_at: at, alive: i % 9 !== 4, progress: i % 3 === 0 ? { task_id: 'k', title: 'Tarefa', done: i % 4, total: 4 } : null };
  });

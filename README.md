...
`docker/concierge` runs a headless Claude Code session in its own container so the chat can answer questions about the user's machines through the MCP endpoint above. It holds no credential itself: the two Claude account config dirs (`CONCIERGE_PRIMARY_CONFIG_DIR`, `CONCIERGE_SECONDARY_CONFIG_DIR`) are mounted read-write from the host, so the CLI can refresh its own token, and each request carries its own short-lived termhub API token in the MCP config it is given for that run. The container is reachable only on the compose network; `CONCIERGE_SECRET` is the shared secret the app sends in `x-concierge-secret` on every call, and with it unset the container refuses every request. It is not rebuilt by a blue/green deploy, so an in-progress conversation survives an app deploy. The chat counts as configured only when `CONCIERGE_URL`, `CONCIERGE_SECRET` **and** `MCP_URL` are all set — `MCP_URL` is the endpoint the concierge reads the machines through, and it has no fallback on purpose: `PUBLIC_URL` is the app host, which sits behind Cloudflare Access, so a derived `/mcp` would answer an Access redirect instead of MCP. With any of the three missing, every message is answered `503 CONCIERGE_DISABLED` with a pt-BR explanation shown on the chat screen, and nothing is stored as a failed answer; if the container is up but refuses or drops the request, the answer is `502 CONCIERGE_FAILED`. A run that starts and then dies is different: it is stored on the assistant message with an `error_code`, and the screen says the answer did not finish.

The token minted for each run carries `read`, `tasks` and `terminals` — the concierge can read anything its owner can see and can also act — but it is always minted `gated: true`, and `gated` is what a person's own MCP token (the one Settings hands out) never is: nobody has to confirm their own keystrokes. On a gated token, a read (`list_machines`, `read_screen`, `list_tasks`, …) still runs immediately; a write or an irreversible call (`send_input`, `run_command`, `create_task`, `close_tab`, `delete_task`, …) is recorded as a pending action instead of run, and the chat shows a card asking the user to confirm or deny it. Confirming re-issues the exact call for real execution; denying tells the model so, and the model does not retry it. A question the user never answers expires after 24h — the hourly timer that also purges expired sessions (`app.ts`) marks it `expired` rather than leaving it open forever, and the same proposal can be asked again after that.

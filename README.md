...
The landing's `VITE_FIREBASE_*` variables are read from that same `.env` and passed to the image **at build time** (compose `build.args`), so the landing image has to be rebuilt after changing them — the deploy workflow already does `up -d --build --no-deps landing`.
